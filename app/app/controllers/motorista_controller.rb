class MotoristaController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @motoristum.update(motoristum_params)
          format.html { redirect_to @motoristum, notice: 'Motorista atualizado com sucesso.' }
  ...
  def destroy
    @motoristum.destroy
    respond_to do |format|
        format.html { redirect_to motorista_url, notice: 'Motorista deletado com sucesso.' }
  ...
  private
    ...
    def motoristum_params
      params.require(:motoristum).permit(:nome, :cpf, :tipo_caminhao, :latitude, :longitude)
    end
end
